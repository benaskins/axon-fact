...
Event sourcing primitives for Go. An append-only event store, synchronous projectors for building read models, and an async publisher interface for streaming events to external consumers. Zero external dependencies — adapters for Postgres and NATS are separate modules.
